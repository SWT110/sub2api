export default {
    ...
    users: {
      ...
      platformQuota: {
        ...
        weeklyStartAt: '周窗口起始时间（可选）',
        weeklyStartAtHint: '按浏览器本地时区解释。用于重置周窗口时留空则以重置时刻为起点；点“仅调起点”会保留已用周额度。',
        weeklyStartAtInvalid: '请输入有效的周窗口起始时间',
        adjustWeeklyStart: {
          button: '仅调起点',
          title: '只修改周窗口起始时间，保留已用周额度',
          required: '请先填写周窗口起始时间',
          future: '周窗口起始时间不能晚于当前时间',
          expired: '周窗口起始时间必须在最近 7 天内，否则额度会立即到期',
          confirm: '确认仅调整用户在 {platform} 平台的周窗口起始时间？已用周额度会保留。',
          success: '已调整 {platform} 周窗口起始时间，已用额度已保留',
          failed: '调整周窗口起始时间失败',
        },
      },
      weeklyQuotaSync: {
        ...
        enabled: '自动同步',
        sourceAccount: '监控账号',
        sourcePlaceholder: '选择 OpenAI OAuth 账号',
        sourceHint: '仅显示非影子 OpenAI OAuth 账号；失效账号不可启用。',
        pollInterval: '检查间隔（秒）',
        pollIntervalHint: '允许范围：60 - 3600 秒',
        invalidPollInterval: '检查间隔必须是 60 到 3600 之间的整数秒',
        sourceRequired: '启用自动同步前，请选择监控账号',
        status: '同步状态',
        observedResetAt: '最近观测的上游下次重置时间',
        observedWeeklyUsedPercent: '最近观测的上游 7 天用量',
        peakWeeklyUsedPercent: '当前周期上游用量峰值',
        pendingSignal: '等待官方重置时间校准的窗口起点',
        pendingResetAt: '等待官方重置时间校准的上游下次重置时间',
        signalResetTime: '上游下次重置时间变化',
        signalUsagePercent: '上游 7 天用量归零',
        signalUsagePercentDrop: '上游 7 天用量大幅下降',
        signalWindowStartCalibration: '官方重置时间校准窗口起点',
        windowStart: '最近同步窗口起点',
        lastCheckedAt: '最近检查时间',
        lastTriggeredAt: '最近自动重置时间',
        lastTriggerSignal: '上次自动重置依据',
        pendingSourceAccountRecovery: '等待恢复监控账号限流状态',
        pendingSourceAccountRecoveryHint: '上一次自动恢复未完成，系统会在后续检查中重试，不会重复重置用户额度。',
        lastSourceAccountRecoveredAt: '最近自动恢复监控账号',
        lastAffectedUsers: '上次受影响用户',
        lastError: '最近错误',
        notAvailable: '-',
        checkNow: '立即检查',
        checking: '检查中...',
        checkRequiresSavedSource: '请先保存已选择的监控账号，再执行检查',
        checkBaseline: '已记录当前上游周额度状态作为基线',
        checkPending: '等待官方重置时间出现后校准窗口起点',
        checkReset: '已检测到官方重置，已同步 {count} 个用户',
        checkResetAndRecovered: '已检测到官方重置，已同步 {count} 个用户并恢复监控账号限流状态',
        checkSourceAccountRecovered: '已恢复监控账号限流状态',
        checkWindowStartAligned: '已校准 {count} 个用户的周窗口起点，已用额度保持不变',
        checkComplete: '检查完成，未检测到新的官方重置',
        manualAnchor: '手动批量锚定',
        manualAnchorTime: '窗口起始时间',
        resetAll: '重置全部用户周额度',
        resetting: '重置中...',
        resetConfirm: '确认以此时间重置所有已配置 OpenAI 周额度的用户？',
        resetSuccess: '已重置 {count} 个用户的周额度',
        invalidAnchorTime: '请输入有效的窗口起始时间',
        futureAnchorTime: '窗口起始时间不能晚于当前时间',
        save: '保存配置',
        saving: '保存中...',
        saveSuccess: '周额度同步配置已保存',
        loadFailed: '加载周额度同步配置失败',
        saveFailed: '保存周额度同步配置失败',
        checkFailed: '检查上游周额度状态失败',
        resetFailed: '批量重置周额度失败',
      }
    },
